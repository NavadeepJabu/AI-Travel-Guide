...
interface HeaderProps {
  ...
  onAITravelPlannerClick?: () => void;
}

const Header: React.FC<HeaderProps> = ({ onSignInClick, onWishlistClick, onPlansClick, onFindGuides, onGuideBookings, onAITravelPlannerClick }) => {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center justify-between py-4">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            ...
            <button onClick={onAITravelPlannerClick} className={`px-4 py-2 rounded-full font-bold transition-all transform hover:scale-105 ${atTop ? 'bg-gradient-to-r from-emerald-400 to-teal-400 text-slate-900 shadow-[0_0_15px_rgba(52,211,153,0.5)]' : 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-lg'}`}>
        ...
        {isMenuOpen && (
          <div className={`lg:hidden py-4 border-t ${atTop ? 'border-white/20' : 'border-gray-200'}`}>
            <nav className="flex flex-col space-y-4">
              ...
              <button onClick={onAITravelPlannerClick} className={`text-left transition-colors font-bold px-2 py-1 ${atTop ? 'text-emerald-400 drop-shadow-md' : 'text-emerald-600'}`}>✨ AI Planner</button>
              ...
              {user ? (
                <div className="space-y-4 pt-4 border-t">
                  ...
                  {/* Toggle: Safety popup sound */}
                  <button
                    onClick={() => {
                      try {
                        const cur = localStorage.getItem('safetySoundEnabled');
                        const next = cur === 'false' ? 'true' : 'false';
                        localStorage.setItem('safetySoundEnabled', next);
                        alert(next === 'true' ? 'Safety sound enabled' : 'Safety sound disabled');
                      } catch { }
                    }}
                    className={`w-full text-left px-3 py-2 rounded ${atTop ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    Toggle Safety Sound
                  </button>
                  {user.role === 'guide' && (
